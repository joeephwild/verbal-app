...
const manifest_storage_layout_1 = require("../core/manifest-storage-layout");
const manifest_1 = require("../core/manifest");
const proxy_kind_1 = require("../core/proxy-kind");
async function processProxyImpl(deployData, proxyAddress, opts) {
    await (0, proxy_kind_1.processProxyKind)(deployData.provider, proxyAddress, opts, deployData.validations, deployData.version);
...
async function validateImpl(deployData, opts, currentImplAddress) {
    ...
    if (currentImplAddress !== undefined) {
        const manifest = await manifest_1.Manifest.forNetwork(deployData.provider);
        const currentLayout = await (0, manifest_storage_layout_1.getStorageLayoutForAddress)(manifest, deployData.validations, currentImplAddress);
