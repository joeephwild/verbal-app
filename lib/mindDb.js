...
export default mindDbQueryCall = async (userId, message) => {
  ...
  try {
    ...
    const query = `SELECT * FROM mindsdb.mindsdb.tutor_model WHERE author_username = '${userId}' AND text = '${message} USING prompt_template=${`Act like a chat buddy to ${authorUsername}, be friendly, act like he was chatting with friend while also teaching him a particular language he requested to learn and also go indept into the topic been asked, making the chat friendly and educational and the same time`}'`;
