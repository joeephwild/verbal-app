...
export async function getCommunityParticipant(communityId) {
  ...
  if (error) {
    alert("Error checking existing entry:", error.message);
    return;
  }

  if (existingEntry && existingEntry.length > 0) {
    alert("number of participant", existingEntry.length);
    return;
  }
}
...
export async function addUserToCommunityWithValidation(userId, communityId) {
  ...
  if (existingEntry && existingEntry.length > 0) {
    alert("User is already a member of this community.");
    return;
  }
  ...
  if (insertError) {
    alert("Error adding user to community:", insertError.message);
    return;
  }

  if (data) {
    alert("User added to community successfully:", data);
  }
}

export async function createPost(user_id, community_id, title, content) {
  ...
  const { data: postData, insertError } = await supabase
    .from("posts")
    .insert([{ user_id, community_id, title, content }]);
